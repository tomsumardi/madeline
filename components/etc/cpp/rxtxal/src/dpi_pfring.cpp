...
MSTS pfringDPI::init()
{
    ...
    do
    {
        ...
        if(ippsDoc.HasMember("interfaces_in"))
        {
            ...
            pLog->debug("interfaces_in/snaplen: {}",_nSnapLength);
            pLog->debug("interfaces_in/direction: {}",_strDirection);
            pLog->debug("interfaces_in/core_bind_id: {}",_nBindId);
            pLog->debug("interfaces_in/watermark: {}",_nWatermark);
            pLog->debug("interfaces_in/poll_wait_msec: {}",_nPollWaitMsec);
            pLog->debug("interfaces_in/ring_cluster_id: {}",_nRingClusterId);
            pLog->debug("interfaces_in/hw_timestamp: {}",_bHwTimeStamp);
            pLog->debug("interfaces_in/strip_timestamp: {}",_bStripTimeStamp);
            pLog->debug("interfaces_in/name: {}",_strIntfName);
            pLog->debug("interfaces_in/pcap_filters: {}",_strPcapFilter);
            ...
            if(!pdIn)
            {
                pLog->error("failure to perform pfring_open on intf {}",_strIntfName);
                break;
            }
            u_int32_t _version;
            ...
            else
            {
                ...
                pLog->info("Capturing from {} [mac: {}][if_index: {}][speed: {}Mb/s]",
                        (char*)_strIntfName.c_str(), etheraddrString(_mac_address, _buf),
                   _ifindex,
                       pfring_get_interface_speed(pdIn));
            }

            pLog->info("# Device RX channels: {}", pfring_get_num_rx_channels(pdIn));
            pLog->info("# Polling threads:    {}", _nThreads);
            ...
            {
                pLog->error("pfring_set_bpf_filter({}) returned {}", _strPcapFilter.c_str(), _intfSts);
                break;
            }
            else
                pLog->info("Successfully set BPF filter '{}'", _strPcapFilter.c_str());
            ...
            {
                pLog->error("pfring_set_direction, options are rx,tx and rx+tx");
                break;
            }
            ...
            {
                pLog->error("pfring_set_socket_mode returned [_intfSts={}]", _intfSts);
                break;
            }
            ...
            if(_nWatermark > 0)
            {
              ...
              if(MDSUCCESS != _intfSts)
              {
                  pLog->error("pfring_set_poll_watermark returned [rc={}][watermark={}]", _intfSts, _nWatermark);
                  break;
              }
            }
            else
            {
                pLog->error("pfring_set_poll_watermark must be > 0");
                break;
            }
            if(_nRingClusterId > 0)
            {
                ...
                if(MDSUCCESS != _intfSts)
                {
                    pLog->error("pfring_set_cluster returned {}", _intfSts);
                }
            }
            if(_nPollWaitMsec > 0)
            ...
            {
                pLog->error("pfring enable ring failure");
                break;
            }
        }
        ...
        if(ippsDoc.HasMember("interfaces_out"))
        {
            ...
            pLog->debug("interfaces_out/snaplen: {}",_nSnapLength);
            pLog->debug("interfaces_out/direction: {}",_strDirection);
            pLog->debug("interfaces_out/core_bind_id: {}",_nBindId);
            pLog->debug("interfaces_out/watermark: {}",_nWatermark);
            pLog->debug("interfaces_out/poll_wait_msec: {}",_nPollWaitMsec);
            pLog->debug("interfaces_out/ring_cluster_id: {}",_nRingClusterId);
            pLog->debug("interfaces_out/hw_timestamp: {}",_bHwTimeStamp);
            pLog->debug("interfaces_out/strip_timestamp: {}",_bStripTimeStamp);
            pLog->debug("interfaces_out/name: {}",_strIntfName);
            ...
            /*if(_nWatermark > 0)
            {
                ...
                if(MDSUCCESS != _intfSts)
                {
                    pLog->error("pfring_set_tx_watermark() failed [rc={}]", _intfSts);
                    break;
                }
            }*/
            if(_nRingClusterId > 0)
            ...
            {
                pLog->error("Unable to enable egress ring");
                break;
            }
        }
        _sts = MDSUCCESS;
...
MSTS pfringDPI::open()
{
  pLog->info("pfringDPI::open()");
  return MDSUCCESS;
}

MSTS pfringDPI::write()
{
  pLog->info("pfringDPI::write()");
  return MDSUCCESS;
}

MSTS pfringDPI::read()
{
  pLog->info("pfringDPI::read()");
  return MDSUCCESS;
}

MSTS pfringDPI::close()
{
  pLog->info("pfringDPI::close()");
  pfring_close(pdIn);
  return MDSUCCESS;
}
