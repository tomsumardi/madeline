...
Madeline is high performance packet router/recorder that can be used to perform OTT Video caching, which includes simple REST API endpoint, packet routing engine and video caching server.
...
### Summary:
Madeline is out-of/in-band inline (realtime) packet router/recorder that can be used to perform OTT Video (MPEG-DASH/apple HLS/etc) caching based on regular expression or ip address/port tuple logic rulesets. 
